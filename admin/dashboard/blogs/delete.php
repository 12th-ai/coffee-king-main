<?php
    ...
    if(isset($_GET['dropCommentId'])){
        ...
        $qq = mysqli_query($conn, "SELECT * FROM comments WHERE ID = $id");
        $qqrun = mysqli_fetch_array($qq);
        $postid = $qqrun['PostID'];
        $blogup = mysqli_query($conn, "SELECT * FROM blog WHERE ID = $postid");
        $bloguprun = mysqli_fetch_array($blogup);
        $blogcmt = $bloguprun['Comments'];
        $newblog = $blogcmt - 1;
        $blognew = mysqli_query($conn, "UPDATE blog SET Comments = $newblog WHERE ID = $postid");
    ...
    if(isset($_GET['dropReplyId'])){
        $id = $_GET['dropReplyId'];
        $sql1 = mysqli_query($conn, "DELETE FROM replies WHERE ID = $id");
