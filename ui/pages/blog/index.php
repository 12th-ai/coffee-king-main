        ...
        <div class="home-mini-cont container">
            <h1>
                ...
                if (isset($_GET['author'])) {
                    $author = $_GET['author'];
                    $get = mysqli_query($conn, "SELECT * FROM team WHERE ID = $author");
                    if(mysqli_num_rows($get) > 0) {
                        $fetch = mysqli_fetch_array($get);
                        $writer = $fetch['Names'];
                        $sql2 = mysqli_query($conn, "SELECT * FROM blog WHERE `Status` = 1 AND Journalist = $author ORDER BY ID DESC");
                        echo "Blogs By " . $writer . " (" . mysqli_num_rows($sql2) . ")";
                    }else{
                        echo "Nothing Found";
                        echo "<script>window.location.href='./pg/?error'</script>";
                    }
                }else if(isset($_GET['date'])) {
                    $date = $_GET['date'];
                    $get = mysqli_query($conn, "SELECT * FROM blog WHERE `Date` LIKE '$date' AND `Status` = 1");
                    if (mysqli_num_rows($get) > 0) {
                        echo "Blogs On " . $date . " (" . mysqli_num_rows($get) . ")";
                    } else {
                        echo "Nothing Found";
                        echo "<script>window.location.href='./pg/?error'</script>";
                    }
            ...
            <div class="home-mini-links">
                ...
                <p>
                    ...
                    if (isset($_GET['author'])) {
                        ...
                        $date = $_GET['date'];
                        $get = mysqli_query($conn, "SELECT * FROM blog WHERE `Date` LIKE '$date'");
                        if (mysqli_num_rows($get) > 0) {
                            echo "Blogs : " . $date;
                        } else {
                            echo "Nothing Found";
            ...
            ?>
                ...
                <a href='../about/team/member/?member=<?php echo $author ?>' class='more more-top'>Portofolio</a>
            <?php }} ?>
        </div>
    </div>
    ...
    <div class="blog">
        <div class="blog-cont container">
            ...
            <div class="blog-cont-crds">
                ...
                if (isset($_GET['author'])) {
                    $author = $_GET['author'];
                    $sql = mysqli_query($conn, "SELECT * FROM blog WHERE `Status` = 1 AND Journalist = $author ORDER BY ID DESC");
                }else if(isset($_GET['date'])) {
                    $sql = mysqli_query($conn, "SELECT * FROM blog WHERE `Date` LIKE '$date' AND `Status` = 1 ORDER BY ID DESC");
                }
                 else {
                    $sql = mysqli_query($conn, "SELECT * FROM blog WHERE `Status` = 1 ORDER BY ID DESC");
                }
                ...
                ?>
                        ...
                        <a href='./pg/?blog=<?php echo $blog ?>' class="blog-cont-crd">
                            <div class="blog-cont-crd-img">
                                <img src="../../assets/blog/<?php echo $pic ?>" alt="">
                            </div>
                            <h1><?php echo $title ?></h1>
                            <p class="blog-cont-txt">
                                <?php echo $summary ?>
                            </p>
                    ...
                    <p>No Blog Posts Available, Stay Tuned</p>
                <?php } ?>
            </div>
        </div>
    </div>
        ...
        <div class="footer-cont container">
            ...
            <div class="footer-cont-scnd footer-crds">
                ...
                <div class="footer-cont-scnd-blw">
                    <a href="../../">Home</a>
                    <a href="#" class="footeractive">Blog</a>
