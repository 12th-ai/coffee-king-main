...
if (isset($_GET['category'])) {
    ...
    $cat = mysqli_query($conn, "SELECT * FROM pro_categories WHERE ID = $catid");
    $catfetch = mysqli_fetch_array($cat);
    $catname = $catfetch['Category_Name'];

    $sql = mysqli_query($conn, "SELECT * FROM product WHERE Categories = $catid");
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Coffee-King || <?php echo $catname ?></title>
    <link rel="shortcut icon" href="../../../.../../../assets/09/cropped-favicon-100x100.png" type="image/x-icon">
    <link rel="stylesheet" href="../../../../css/explore/shop/main.css">
</head>
...
<body>
    ...
    <div class="product-main">
        <div class="product-cont container">
            <?php include "../sidenav_2.php";
            $sql = mysqli_query($conn, "SELECT * FROM product WHERE Categories = $catid");
            if (mysqli_num_rows($sql) == 0) {
            ?>
                <p id="noProducts">No Products In This Category Yet</p>
            <?php } else { ?>
                <div class="product-cont-right">
                    <?php
                    $prod = mysqli_query($conn, "SELECT * FROM product WHERE Categories = $catid");
                    while ($prodfetch = mysqli_fetch_array($prod)) {
                        $prodid = $prodfetch['ID'];
                        $prodname = $prodfetch['Product_Name'];
                        $prodprice = $prodfetch['Product_Price'];
                        $prodBrief = $prodfetch['Brief'];
                        $prodprice = $prodfetch['Product_Price'];
                        $prodDisc = $prodfetch['Product_Discount'];
                        $prodimg = $prodfetch['Main_Preview'];
                        $prodstock = $prodfetch['Quantity'];
                    ?>

                        <div class="product-crd">
                            <div class="product-crd-img">
                                <img src="../../../../assets/09/<?php echo $prodimg ?>" alt="">
                            </div>
                            <a href='../product/?product="<?php echo $prodid ?>"'><?php echo $prodname ?></a>
                            <p><?php echo $prodBrief ?></p>
                            <div class="product-crd-price">
                                <?php
                                if ($prodprice == $prodDisc) {
                                ?>
                                    <h1>$<?php echo $prodprice ?></h1>
                                <?php } else { ?>
                                    <del>$<?php echo $prodprice ?></del>
                                    <h1>$<?php echo $prodDisc ?></h1>
                                <?php } ?>
                            </div>
                            <button class="more cart-btn" <?php if ($prodstock <= 0) {
                                                                echo "disabled";
                                                            } ?>>
                                <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="512.000000pt" height="512.000000pt" viewBox="0 0 512.000000 512.000000" preserveAspectRatio="xMidYMid meet">
                                    <g transform="translate(0.000000,512.000000) scale(0.100000,-0.100000)" stroke="none">
